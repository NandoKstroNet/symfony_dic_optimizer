...
{
	zend_function *get, *camelize, *has;
	...
	if (zend_hash_find(&ce->function_table, "has", sizeof("has"), (void **)&has) == FAILURE) {
		return FAILURE;
	}

	zend_function_dtor(get);
	zend_function_dtor(camelize);
	zend_function_dtor(has);
	...
	has->type = ZEND_INTERNAL_FUNCTION;
	has->internal_function.handler = dic_optimizer_has_handler;
	has->internal_function.module  = &symfony_dic_module_entry;
	has->internal_function.arg_info = (zend_arg_info *)dic_optimizer_camelize_arg_infos;
